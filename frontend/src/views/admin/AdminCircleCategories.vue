...
import {
  addAdminCircleCategory,
  deleteAdminCircleCategory,
  getAdminCircleCategories,
  updateAdminCircleCategory,
} from '../../api/admin'
...
const load = async () => {
  try {
    const res = await getAdminCircleCategories()
...
const handleSave = async () => {
  const res = form.id ? await updateAdminCircleCategory(form) : await addAdminCircleCategory(form)
  if (res.code !== 200) {
...
const handleDelete = async (id) => {
  const res = await deleteAdminCircleCategory(id)
  if (res.code !== 200) {
